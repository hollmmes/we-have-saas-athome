...
export async function loadDatabaseStats() {
  try {
    ...
    document.getElementById("dbImageCount")!.textContent =
      images.length.toString();
    document.getElementById("dbVideoCount")!.textContent =
      videos.length.toString();
    document.getElementById("dbDomainCount")!.textContent =
      domains.length.toString();

    document.getElementById("tableImageCount")!.textContent =
      `${images.length} kayit`;
    document.getElementById("tableVideoCount")!.textContent =
      `${videos.length} kayit`;
    document.getElementById("tableDomainCount")!.textContent =
      `${domains.length} kayit`;
    ...
    try {
      ...
      try {
        ...
        const estimatedSize =
          images.length * 0.5 + videos.length * 0.5 + domains.length * 0.3;
...
export function setupDatabasePage() {
  ...
  refreshDbStatsBtn.addEventListener("click", async () => {
    ...
    alert("Istatistikler guncellendi!");
  });
}
