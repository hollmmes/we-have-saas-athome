...
function updateCropBox() {
  ...
  if (cropImage && cropCanvas && cropOverlay) {
    ...
    if (cropDimensions) {
      cropDimensions.textContent = `${actualWidth} x ${actualHeight}`;
    }

    if (cropRatio) {
      if (cropState.aspectRatio > 0) {
        const activeTemplate = document.querySelector(".template-btn.active");
        cropRatio.textContent =
          activeTemplate.querySelector("span").textContent || "Sabit Oran";
      } else {
...
function saveCroppedImage() {
  ...
  const fileName = `cropped-${outputWidth}x${outputHeight}.jpg`;
  void saveCanvasToOutput(outputCanvas, fileName, "cropped_images", 0.95)
    .then(() => {
      alert(
        `Fotograf basariyla kirpildi ve cropped_images klasorune kaydedildi: ${fileName}`,
      );
    })
    .catch((error) => {
      console.error("Crop save error:", error);
      downloadCanvas(outputCanvas, fileName, 0.95);
